...
if(option.equals("checkAverageSeverity"))
{
	...
	List<String> averageSeverityList = (List<String>) request.getAttribute("averageSeverityList");
	@SuppressWarnings("unchecked")
	...
	<table>
		...
		<%
	for(int i=0; i< averageSeverityList.size(); i++) {
		%>

			<tr>
			<th colspan="2"><%= priorities.get(i).getPriorityName()%>  </th>
			</tr>
...
else if(option.equals("getReportStatisticsPerServiceEngineer"))
{
	...
	for(int i=0; i< statistics.size(); i++) {
		%>
			...
			<tr>
			<th colspan="2"><%= engineerDetails.get(i).getCredentials().getUsername()%>  </th>
			</tr>
...
for(i=0; i< ageOfOpenTickets.size(); i++) {
	%>
		...
		<tr>
		<th colspan="2">TicketID-<%= ageOfOpenTickets.get(i)[0]%>  </th>
		</tr>
...
request.removeAttribute("averageSeverityList");
